...
def run(target_server):
    ...
    for i in range(0, NUM_SERVERS):  # dont start on uph
        ...
        # time.sleep(5)
        print("\t > Starting pinger on %s..." % net.hosts[i].name)
        # net.hosts[i].sendCmd("ping %s" % target_server)
        net.hosts[i].sendCmd(
            "/home/grads/qkb5007/Documents/repos/floodlight/scripts/pinger.sh %d > pinger-output/h%d.log &" % (i+1, i+1))
        # net.hosts[i].cmdPrint( 'tc -s qdisc ls dev', net.hosts[i].defaultIntf() )
        # net.switches[i].cmd(
        #     "/home/grads/qkb5007/Documents/repos/floodlight/scripts/pinger.sh %d > sw%d.log &" % (i, i))
        # print("\t Waiting for floodight statistics service to collect new stats for next client...\n")
        # if i < (len(net.hosts)-2-1):
        #     time.sleep(15)
    print("[Done]\n")
